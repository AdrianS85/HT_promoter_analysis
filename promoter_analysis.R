...
# BS_homo <- BSgenome.Hsapiens.UCSC.hg38::BSgenome.Hsapiens.UCSC.hg38 # This is a genome sequence from UCSC
BS_homo <- BSgenome.Hsapiens.UCSC.hg19::BSgenome.Hsapiens.UCSC.hg19 
# TxDb_homo <- TxDb.Hsapiens.UCSC.hg38.knownGene::TxDb.Hsapiens.UCSC.hg38.knownGene # This is the annotation file from UCSC
...
### This returns IRangnes for every transcript of the gene. It does it correctly.
...
### This returns composite sequence generated by adding promoters for every transcript
...
dataset_length <- length(gff_of_unlist_promoter_sequence[,1])
dataset_mean_score <- mean(gff_of_unlist_promoter_sequence$score)
dataset_mean_start <- mean(gff_of_unlist_promoter_sequence$start)
...
randoms_lengths_vector <- purrr::map_int(.x = gff_of_unlisted_random_promoter_sequences_for_sampling_distribution, .f = function(x) { length(x[[1]]) })

randoms_mean_scores_vector <- purrr::map_dbl(.x = gff_of_unlisted_random_promoter_sequences_for_sampling_distribution, .f = function(x) { mean(x$score) })
mean(randoms_mean_scores_vector)

randoms_mean_start_vector <- purrr::map_dbl(.x = gff_of_unlisted_random_promoter_sequences_for_sampling_distribution, .f = function(x) { mean(x$start) })
...
# test3 <- get_and_write_gff_from_DNAStringSet_via_JASPAR(DNAStringSet_ = unlisted_random_promoter_sequences_for_sampling_distribution[[2]], input_genes_ = sample_list_for_sampling_distribution[[2]], opts_species = 9606, opts_name = "random", input_col_with_entrezID = 'NCBI Gene ID', input_col_with_geneName = 'Approved symbol') # This returns zero metrices in getMatrixSet function
